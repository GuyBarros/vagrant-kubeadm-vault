...
# ETCD
## ETCD CA Cert
vault secrets enable -path=etcd_root pki
vault secrets tune -max-lease-ttl=87600h etcd_root
vault write -format=json etcd_root/root/generate/exported common_name="etcd-ca" ttl=315360000s > ca.json
...
vault secrets enable -path= etcd_int pki
vault secrets tune -max-lease-ttl=43800h  etcd_int
vault write -format=json  etcd_int/intermediate/generate/internal \
        common_name=" etcd-ca" \
        | jq -r '.data.csr' > etcd_pki_intermediate.csr

vault write -format=json  etcd_root/root/sign-intermediate csr=@etcd_pki_intermediate.csr \
        format=pem_bundle ttl="43800h" \
        | jq -r '.data.certificate' > etcd_intermediate.cert.pem
...
# Proxy
## Proxy CA Cert
vault secrets enable -path=proxy_root pki
vault secrets tune -max-lease-ttl=87600h proxy_root
vault write -format=json proxy_root/root/generate/exported common_name="kubernetes-front-proxy-ca" ttl=315360000s > ca.json
...
## ETCD Intermediate CA
vault secrets enable -path= proxy_int pki
vault secrets tune -max-lease-ttl=43800h  proxy_int
vault write -format=json  proxy_int/intermediate/generate/internal \
        common_name=" kubernetes-front-proxy-ca" \
        | jq -r '.data.csr' > proxy_int_intermediate.csr

vault write -format=json  proxy_root/root/sign-intermediate csr=@proxy_int_intermediate.csr \
        format=pem_bundle ttl="43800h" \
        | jq -r '.data.certificate' > proxy_intermediate.cert.pem

vault write proxy_int/intermediate/set-signed certificate=@proxy_intermediate.cert.pem

##Roles Proxy CA
vault write proxy_int/roles/proxy-ca \
        allow_bare_domains=true \
        allow_subdomains=true \
        allow_glob_domains=true \
        allow_any_name=true \
